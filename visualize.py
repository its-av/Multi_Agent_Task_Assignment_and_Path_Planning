...
class Animation:
    def __init__(self, my_map, starts, goals, paths,tasks,fin_ta):
        ...
        self.fin_ta=[]
        for task in tasks:
            self.tasks.append((task[1], len(self.my_map[0]) - 1 - task[0]))
        for f in fin_ta:
            self.fin_ta.append((f[1], len(self.my_map[0]) - 1 - f[0]))
        if paths:
            for path in paths:
                self.paths.append([])
                for loc in path:
        ...
        for i, task in enumerate(self.tasks):
            self.patches.append(Rectangle((task[0] - 0.25, task[1] - 0.25), 0.5, 0.5, facecolor='red',
                                          edgecolor='red', alpha=1))
            self.task_patches[(task[0], task[1])] = {
    ...
    def init_func(self):
        ...
        self.tasks_picked = []
        for k, v in self.task_patches.items():
    ...
    def animate_func(self, t):
        for k in range(len(self.paths)):
            ...
            if tuple(pos) in self.task_patches.keys() and tuple(pos) not in self.tasks_picked:
                # print (tasks_picked)
                v = self.task_patches[tuple(pos)]
                print ("Task %d picked!"%v['task_id'])
                self.patches[v['idx']].set_visible(False)
                self.tasks_picked.append(tuple(pos))
